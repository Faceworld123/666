...
if(!isset($id))
{
header ("location: http://10.1.10.223:8080/index.php");	
}
...
for ($i = 0; $i < $length; $i++) {
  ...
  $asc = (char)$string[$i];
                    ...
                    if((n - 1) % 3 == 0) Encrypt($string[$i], $counter, $matrix, $key1, $key2, $key3, $key4, $key5, $key6, $asc);
                    if((n - 1) % 3 == 1) Encrypt($string[$i], $counter, $matrix, $key7, $key8, $key9, $key10, $key11, $key12, $asc);
                    if((n - 1) % 3 == 2) Encrypt($string[$i], $counter, $matrix, $key13, $key14, $key15, $key16, $key17, $key18, $asc);

                   // $code = n++;
                 //   $textbox23 = (string)$code;
...
<input type="text" id="email" name="user_email" value="<?php echo htmlspecialchars($key8); ?>" >
 <input type="text" id="email" name="user_email" value="<?php echo htmlspecialchars($key9); ?>">   
    ...
    <textarea id="encrypt" maxlength="140" name="encrypt" rows="4" cols="50" placeholder="Enter Plaintext Data:"></textarea>
<br><br>    
        <textarea id="decrypt" name="decrypt" rows="4" cols="50" placeholder="Ciphertext Data:"></textarea>
    <br><br>
